...
#' @param topic_model List. Structural topic model as returned by the function text_model_topics.
...
text_apply_stm <- function(bow,
  ...
  apply_tm <- stm::fitNewDocuments(model = topic_model$topic_model, documents = aligneddoc$documents, newData=aligneddoc$meta)
  ...
  if (!is.null(keywords)){
    ...
    prediction <- prediction %>%
      dplyr::left_join(dplyr::select(topic_model$topic_term, topic, term), by = "topic") %>%
