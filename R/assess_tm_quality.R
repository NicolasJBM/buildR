...
assess_tm_quality <- function(
  ...
  heldout <- stm::make.heldout(doc_to_term)
  ...
  tm_quality <- expand.grid(topic_nbr, seeds) %>%
    ...
    dplyr::mutate(
      ...
      residuals = purrr::map(model, stm::checkResiduals, doc_to_term),
      semantic_coherence = purrr::map(model, stm::semanticCoherence, documents = doc_to_term),
  ...
  if (length(keywords)>1){
    ...
    assess_overlap <- function(topic_nbr,  seed, model, topic_per_doc, terms_per_topic, keywords){
      ...
      ref_topics <- tidytext::tidy(model, matrix = "gamma", document_names = rownames(doc_to_term)) %>%
